/**
 * Función Lambda para obtener todas las cuotas desde Amazon Athena
 * GET /cuotas
 */
...
const waitForQueryToComplete = async (queryExecutionId) => {
  console.log(`[DEBUG] Iniciando waitForQueryToComplete para QueryExecutionId: ${queryExecutionId}`);
  ...
  while (attempts < maxAttempts) {
    ...
    console.log(`[DEBUG] Intento ${attempts + 1} de ${maxAttempts} para verificar estado de la consulta`);
    ...
    try {
      console.log(`[DEBUG] Llamando a athena.getQueryExecution con params:`, JSON.stringify(params));
      ...
      const submissionDateTime = result.QueryExecution.Status.SubmissionDateTime;
      const completionDateTime = result.QueryExecution.Status.CompletionDateTime;
      ...
      if (result.QueryExecution.Statistics) {
        console.log(`[DEBUG] Estadísticas disponibles:`, JSON.stringify(result.QueryExecution.Statistics, null, 2));
      }

      if (state === "SUCCEEDED") {
        console.log(`[DEBUG] Consulta completada exitosamente después de ${attempts + 1} intentos`);
        return result.QueryExecution;
      } else if (state === "FAILED" || state === "CANCELLED") {
        const errorReason = result.QueryExecution.Status.StateChangeReason;
        console.error(`[ERROR] Query failed with state: ${state}. Reason: ${errorReason}`);
        
        // Agregar más información sobre el error
        if (result.QueryExecution.Status.AthenaError) {
          console.error(`[ERROR] Athena Error Details:`, JSON.stringify(result.QueryExecution.Status.AthenaError, null, 2));
        }
      ...
      console.log(`[DEBUG] Consulta aún en estado: ${state}. Esperando 2 segundos...`);
      console.log(`[DEBUG] Incrementando attempts de ${attempts} a ${attempts + 1}`);
      ...
      console.error(`[ERROR] Error checking query status on attempt ${attempts + 1}:`, error);
      console.error(`[ERROR] Error stack:`, error.stack);
      throw new Error(`Error checking query status: ${error.message}`);
    }
  }

  const timeoutError = "Query timeout: La consulta tardó más de 5 minutos en completarse";
...
const getQueryResults = async (queryExecutionId) => {
  console.log(`[DEBUG] Iniciando getQueryResults para QueryExecutionId: ${queryExecutionId}`);
  ...
  try {
    console.log(`[DEBUG] Llamando a athena.getQueryResults con params:`, JSON.stringify(params));
    const result = await athena.getQueryResults(params).promise();
    
    console.log(`[DEBUG] Resultados obtenidos. Cantidad de filas en ResultSet: ${result.ResultSet.Rows.length}`);
    return result;
  } catch (error) {
    console.error(`[ERROR] Error getting query results for QueryExecutionId ${queryExecutionId}:`, error);
...
const formatResults = (results) => {
  ...
  try {
    ...
    console.log(`[DEBUG] Datos formateados exitosamente. Filas de datos: ${formattedData.length}`);
...
exports.handler = async (event) => {
  try {
    console.log(
      "Evento recibido para getCuotas:",
      JSON.stringify(event, null, 2)
    );

    const { id_departamento } = event.queryStringParameters || {};

    // Construir la consulta dinámicamente
    let query = `SELECT * FROM "${ATHENA_CONFIG.Database}"."g2_cuotas_tb"`;
    const queryClauses = [];

    if (id_departamento) {
      // Importante: Sanitizar la entrada para evitar inyección de SQL
      const sanitizedId = id_departamento.replace(/'/g, "''");
      queryClauses.push(`id_departamento = '${sanitizedId}'`);
    }

    if (queryClauses.length > 0) {
      query += ` WHERE ${queryClauses.join(' AND ')}`;
    }
    
    query += " ORDER BY id_cuota";

    console.log("Ejecutando consulta de cuotas:", query);
    ...
    console.log(`[DEBUG] Configuración de la consulta:`, JSON.stringify(queryParams, null, 2));
    ...
    const processedResults = {
      ...
      cuotas: formattedResults.data,
    };

    console.log(`[DEBUG] Query completado exitosamente. Rows returned: ${formattedResults.totalRows}`);
    ...
    console.error(`[ERROR] Error en getCuotas:`, error);
    console.error(`[ERROR] Stack trace:`, error.stack);

    return response(500, {
      success: false,
      message: "Error al obtener las cuotas desde Athena",
      error: error.message,
      timestamp: new Date().toISOString(),
    });
  }
