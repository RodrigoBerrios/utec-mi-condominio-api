/**
 * Función Lambda para obtener lista de deudores desde Amazon Athena
 * GET /deudores?periodo=2025-01 (opcional)
 */
...
exports.handler = async (event) => {
  try {
    console.log(
      "Evento recibido para getDeudores:",
    ...
    console.log("Ejecutando consulta de deudores:", query);
    ...
    // Procesar los datos de deudores
    const deudores = formattedResults.data.map((row) => ({
    ...
    console.log(
      `[DEBUG] Query completado exitosamente. Total deudores encontrados: ${deudores.length}`
    );
    ...
    console.error(`[ERROR] Error en getDeudores:`, error);
    console.error(`[ERROR] Stack trace:`, error.stack);

    return response(500, {
      success: false,
      message: "Error al obtener la lista de deudores desde Athena",
      error: error.message,
      timestamp: new Date().toISOString(),
    });
  }
