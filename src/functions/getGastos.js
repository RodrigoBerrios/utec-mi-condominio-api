...
const waitForQueryToComplete = async (queryExecutionId) => {
    while (true) {
        try {
            ...
            console.error(`Error esperando la consulta de Athena: ${error.message}`);
            throw error;
        }
    }
};
...
exports.handler = async (event) => {
    console.log('Evento recibido en getGastos:', JSON.stringify(event, null, 2));
    ...
    if (!periodo) {
        return {
            ...
            body: JSON.stringify({ success: false, message: "El parámetro 'periodo' es requerido." }),
        };
    }
    ...
    if (!periodoRegex.test(periodo)) {
        return {
            statusCode: 400,
            headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
            body: JSON.stringify({ success: false, message: "Formato de 'periodo' inválido. Use YYYY-MM." }),
        };
    }

    const query = `
        SELECT date_format(fecha, '%Y-%m') as periodo, * 
        FROM "${ATHENA_DB}"."g2_gastos_tb" 
        WHERE date_format(fecha, '%Y-%m') = '${periodo.replace(/'/g, "''")}'
        ORDER BY id_gasto
    `;
