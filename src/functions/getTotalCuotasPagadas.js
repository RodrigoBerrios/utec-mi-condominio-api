/**
 * Función Lambda para obtener total de cuotas pagadas desde Amazon Athena
 * GET /cuotas-pagadas?periodo=2025-01 (opcional)
 */
...
exports.handler = async (event) => {
  try {
    console.log(
      "Evento recibido para getTotalCuotasPagadas:",
    ...
    console.log("Ejecutando consulta de total cuotas pagadas:", query);
    ...
    return response(200, {
      success: true,
      message: "Total de cuotas pagadas obtenido exitosamente",
      montoAcumulado: parseFloat(montoAcumulado) || 0,
      periodo: periodo || null,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error(`[ERROR] Error en getTotalCuotasPagadas:`, error);
    console.error(`[ERROR] Stack trace:`, error.stack);

    return response(500, {
      success: false,
      message: "Error al obtener el total de cuotas pagadas desde Athena",
      error: error.message,
      timestamp: new Date().toISOString(),
    });
  }
