/**
 * Función Lambda para obtener total de cuotas vencidas desde Amazon Athena
 * GET /cuotas-vencidas?periodo=2025-01 (opcional)
 */
...
exports.handler = async (event) => {
  try {
    console.log(
      "Evento recibido para getTotalCuotasVencidas:",
    ...
    console.log("Ejecutando consulta de total cuotas vencidas:", query);
    ...
    // Extraer el monto acumulado del resultado
    const montoAcumulado =
      formattedResults.data.length > 0
        ? formattedResults.data[0].monto_acumulado
        : "0";
    ...
    const processedResults = {
      ...
      montoAcumulado: parseFloat(montoAcumulado) || 0,
      filtroPeriodo: periodo || null,
    };

    console.log(
      `[DEBUG] Query completado exitosamente. Monto acumulado: ${montoAcumulado}`
    );

    console.log(`[DEBUG] Preparando respuesta exitosa...`);
    return response(200, {
      success: true,
      message: "Total de cuotas vencidas obtenido exitosamente",
      montoAcumulado: parseFloat(montoAcumulado) || 0,
      periodo: periodo || null,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error(`[ERROR] Error en getTotalCuotasVencidas:`, error);
    console.error(`[ERROR] Stack trace:`, error.stack);

    return response(500, {
      success: false,
      message: "Error al obtener el total de cuotas vencidas desde Athena",
      error: error.message,
      timestamp: new Date().toISOString(),
    });
  }
